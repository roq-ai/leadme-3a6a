...
import { getFormSubmissions } from 'apiSdk/form-submissions';
import { FormSubmissionInterface } from 'interfaces/form-submission';
import { getTeamMembers } from 'apiSdk/team-members';
import { TeamMemberInterface } from 'interfaces/team-member';

import { FormSubmissionListPage as ShowCaseList } from 'pages/form-submissions';

function DashboardPage() {
  const { session } = useSession();

  const leadFormStat = useSWR<PaginatedInterface<LeadFormInterface>>(
    () => `/lead-forms`,
    () => getLeadForms({ limit: 1, offset: 0 }),
  );

  const formSubmissionStat = useSWR<PaginatedInterface<FormSubmissionInterface>>(
    () => `/form-submissions`,
    () => getFormSubmissions({ limit: 1, offset: 0 }),
  );

  const teamMemberStat = useSWR<PaginatedInterface<TeamMemberInterface>>(
    () => `/team-members`,
    () => getTeamMembers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Lead Forms',
      result: leadFormStat?.data?.totalCount,
      isLoading: leadFormStat.isLoading,
    },
    {
      title: 'Form Submissions',
      result: formSubmissionStat?.data?.totalCount,
      isLoading: formSubmissionStat.isLoading,
    },
    {
      title: 'Team Members',
      result: teamMemberStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`form-submissions`} passHref legacyBehavior>
                  <Button
