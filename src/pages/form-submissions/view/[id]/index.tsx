...
import { getFormSubmissionById } from 'apiSdk/form-submissions';
import { FormSubmissionInterface } from 'interfaces/form-submission';

function FormSubmissionViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<FormSubmissionInterface>(
    () => (id ? `/form-submissions/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('form_submission', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/form-submissions/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('guest', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Guest"
                    text={
                      <Link as={NextLink} href={`/guests/view/${data?.guest?.id}`}>
                        {data?.guest?.id}
                      </Link>
                    }
                  />
                ...
                {hasAccess('lead_form', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Lead Form"
                    text={
                      <Link as={NextLink} href={`/lead-forms/view/${data?.lead_form?.id}`}>
                        {data?.lead_form?.name}
                      </Link>
                    }
                  />
