...
import { getLeadForms, deleteLeadFormById } from 'apiSdk/lead-forms';
import { LeadFormInterface } from 'interfaces/lead-form';

type ColumnType = ColumnDef<LeadFormInterface, unknown>;

interface LeadFormListPageProps {
...
export function LeadFormListPage(props: LeadFormListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getLeadForms({
        relations: ['organization', 'form_submission.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LeadFormInterface>>(
    () => `/lead-forms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: LeadFormInterface) => {
    if (hasAccess('lead_form', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/lead-forms/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/lead-forms/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('lead_form', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/lead-forms/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('lead_form', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="lead_form" />
        </Flex>

        {hasAccess('lead_form', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/lead-forms/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
