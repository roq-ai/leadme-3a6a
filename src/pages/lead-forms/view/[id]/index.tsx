...
import { getLeadFormById } from 'apiSdk/lead-forms';
import { LeadFormInterface } from 'interfaces/lead-form';
import { FormSubmissionListPage } from 'pages/form-submissions';

function LeadFormViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<LeadFormInterface>(
    () => (id ? `/lead-forms/${id}` : null),
    () =>
      getLeadFormById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('lead_form', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/lead-forms/edit/${id}`} passHref legacyBehavior>
                    <Button
